...
- Always pass `--model gpt-5.5` unless the user explicitly requests a different model. This pins the execution model and prevents the app server from silently falling back to its own default.
...
- If the user asks for a concrete model name such as `gpt-5.4-mini`, pass it through with `--model` (overriding the default pin).
